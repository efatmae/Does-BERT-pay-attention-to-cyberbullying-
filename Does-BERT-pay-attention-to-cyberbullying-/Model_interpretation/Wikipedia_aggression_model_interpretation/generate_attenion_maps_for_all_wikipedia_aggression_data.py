...
def get_model_attention_for_sentence(sentences,single_sentence=False, add_special_token=False):
  ...
  if single_sentence == True:
    token = [tokenizer.cls_token] + tokenizer.tokenize(sentences)[:32] + [tokenizer.sep_token]
    ...
    for sent in sentences:
        token = [tokenizer.cls_token] + tokenizer.tokenize(sent)[:32] + [tokenizer.sep_token]
        inputids = torch.LongTensor([tokenizer.encode(token, add_special_tokens=add_special_token)])
        # Add the encoded sentence to the list.
        outputs = model(inputids)   # Forward pass, calculate logit predictions
        sentences_tokens.append(token)
...
output_dir = "../../trained_models/BERT-Fine-Tuned/Pytorch/Fine_Tune_wtp_agg_clean_text128/"
...
data_test = pd.read_csv("../../../Data/wikipedia_aggression/wp_agg_data_test_sample_1000.csv")
