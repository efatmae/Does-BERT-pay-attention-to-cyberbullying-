...
Kaggle_test_df = pd.read_csv("../../../Data/Kaggle/kaggle_data_test_sample_1000.csv")

#Kaggle_train_df = Kaggle_train_df.dropna()
Kaggle_test_df = Kaggle_test_df.dropna()

print(len(Kaggle_test_df))

reduced_len_sentence = []
for i in Kaggle_test_df.Text_clean_for_BERT.values:
    s = i.split(" ")[:]
    reduced_len_sentence.append(" ".join(s))

Kaggle_test_df["reduced_text_clean"] = reduced_len_sentence
...
model = BertForSequenceClassification.from_pretrained(
    '../../trained_models/BERT-Fine-Tuned/Pytorch/Fine_Tune_Kaggle_clean_text128_no_stem/')
model.to(device)
model.eval()
model.zero_grad()

# load tokenizer
tokenizer = BertTokenizer.from_pretrained('../../trained_models/BERT-Fine-Tuned/Pytorch/Fine_Tune_Kaggle_clean_text128_no_stem/')
...
word_attribution_df.to_csv("attribution_Scores/bert_with_fine_tuning/Kaggle_1000_data_fine_tuned_bert_attribution_scores.csv", index=False)
