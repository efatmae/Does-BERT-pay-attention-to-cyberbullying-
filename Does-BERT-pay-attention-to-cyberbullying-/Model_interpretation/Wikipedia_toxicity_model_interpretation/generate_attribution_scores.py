...
wikipedia_toxicity_test_df = pd.read_csv("../../../Data/wikipedia_toxicity/wp_tox_data_test_sample_1000.csv")

#wikipedia_aggression_train_df = wikipedia_aggression_train_df.dropna()
wikipedia_toxicity_test_df = wikipedia_toxicity_test_df.dropna()

print(len(wikipedia_toxicity_test_df))

reduced_len_sentence = []
for i in wikipedia_toxicity_test_df.Text_clean_for_BERT.values:
    s = i.split(" ")[:]
    reduced_len_sentence.append(" ".join(s))

wikipedia_toxicity_test_df["reduced_text_clean"] = reduced_len_sentence
...
# load model
model = BertForSequenceClassification.from_pretrained('../../trained_models/BERT-Fine-Tuned/Pytorch/Fine_Tune_wtp_tox_clean_text128/')
model.to(device)
model.eval()
model.zero_grad()

# load tokenizer
tokenizer = BertTokenizer.from_pretrained('../../trained_models/BERT-Fine-Tuned/Pytorch/Fine_Tune_wtp_tox_clean_text128/')
...
test_set_word_att_dict = get_attribution_for_test_set(lig, wikipedia_toxicity_test_df)
...
word_attribution_df.to_csv("attribution_scores/bert_with_fine_tuning/wikipedia_toxicity_1000_data_fine_tuned_bert_attribution_scores.csv", index=False)
