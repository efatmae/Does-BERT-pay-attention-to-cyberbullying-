...
Twitter_sexism_test_df = pd.read_csv("../../../Data/Twitter_sexism/Twitter_sex_data_test_sample_1000.csv")

#Kaggle_train_df = Kaggle_train_df.dropna()
Twitter_sexism_test_df = Twitter_sexism_test_df.dropna()

print(len(Twitter_sexism_test_df))

reduced_len_sentence = []
for i in Twitter_sexism_test_df.Text_clean_for_BERT.values:
    s = i.split(" ")[:]
    reduced_len_sentence.append(" ".join(s))

Twitter_sexism_test_df["reduced_text_clean"] = reduced_len_sentence

data_test_pos = Twitter_sexism_test_df[Twitter_sexism_test_df["oh_label"] == 1]
data_test_neg = Twitter_sexism_test_df[Twitter_sexism_test_df["oh_label"] == 0]
...
model = BertForSequenceClassification.from_pretrained(
    '../../trained_models/BERT-Fine-Tuned/Pytorch/Fine_Tune_Twitter_sexism_clean_text_64/')
model.to(device)
model.eval()
model.zero_grad()

# load tokenizer
tokenizer = BertTokenizer.from_pretrained('../../trained_models/BERT-Fine-Tuned/Pytorch/Fine_Tune_Twitter_sexism_clean_text_64/')
...
test_set_word_att_dict = get_attribution_for_test_set(lig, Twitter_sexism_test_df)
...
word_attribution_df.to_csv("attribution_Scores/Fine-tuned_BERT/Twitter_sexism_1000_data_fine_tuned_bert_attribution_scores.csv", index=False)
